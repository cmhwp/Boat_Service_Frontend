<template>
  <div class="goods-edit-view">
    ...
    <div v-else class="error-state">
      <el-result icon="error" title="加载失败" sub-title="无法加载商品信息，请检查网络连接">
        <template #extra>
...
import {
  getMyProductDetailApiV1ProductsMyProductIdGet,
  updateMyProductApiV1ProductsMyProductIdPut,
...
// 表单数据
const form = reactive<API.ProductUpdateSchema>({
  name: '',
  category: 'other',
  description: '',
  price: 0,
...
const fetchProductDetail = async () => {
  try {
    loading.value = true
    const response = await getMyProductDetailApiV1ProductsMyProductIdGet({
    ...
    if (response.data.success && response.data.data) {
      ...
      form.stock = response.data.data.stock
      form.unit = response.data.data.unit
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const response = await updateMyProductApiV1ProductsMyProductIdPut(
      { product_id: productId.value },
      form,
    )
